...
case "$MODE" in
  clipboard)
    # SECURITY: Use XDG_RUNTIME_DIR (tmpfs) instead of /tmp to prevent secrets from persisting on disk
    ...
    # SECURITY: Wrap the async background job in a shell command to ensure the temp file is deleted after use
