...
case "$MODE" in
  clipboard)
    # Ensure temporary file is created in a memory-backed tmpfs if possible,
    # to avoid writing sensitive clipboard contents to disk.
    ...
    # Use sh -c to ensure the temporary file is securely deleted after
    # localsend_app completes, since systemd-run executes asynchronously.
